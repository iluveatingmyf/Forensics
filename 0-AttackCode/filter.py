...
def check_packets():
    ...
    try:
        ...
        print(f"检查iptables失败: {e.stderr}")


def setup_iptables(addr1,addr2,net_if,port):
    existing_nat = iptc.easy.dump_chain('nat','PREROUTING')
    existing_filter = iptc.easy.dump_chain('filter','FORWARD')
    ...
    try:
        #如果数据包的标记（fwmark）为1，则应查找（lookup）编号为100的路由表
        os.system("ip rule add fwmark 1 lookup 100")
        #这个命令在编号为100的路由表中添加了一个默认路由，将流量指向本地回环接口（lo）。这意味着标记为1的所有流量都会被重定向到本机处理
        os.system("ip route add local default dev lo table 100")

        os.system("iptables -t mangle -I PREROUTING -p udp -s 192.168.0.159 -d 192.168.0.192 --dport 0:65535 -j TPROXY --on-port 10000 --tproxy-mark 0x01/0x01")
        os.system("iptables -t mangle -I PREROUTING -p udp -d 192.168.0.159 -s 192.168.0.192 --dport 0:65535 -j TPROXY --on-port 10000 --tproxy-mark 0x01/0x01")
        #设置iptables规则以使用TPROXY
        #os.system("iptables -t mangle -I PREROUTING -p udp -s 192.168.0.159 --dport 0:65535 -j TPROXY --on-port 10000 --tproxy-mark 0x01/0x01")
        #os.system("iptables -t mangle -I PREROUTING -p udp  -d 192.168.0.192 --dport 0:65535 -j TPROXY --on-port 10000 --tproxy-mark 0x01/0x01")
        #os.system("iptables -t mangle -I PREROUTING -p udp -s 192.168.0.159 --dport 0:65535 -j TPROXY --on-port 10000 --tproxy-mark 0x01/0x01")
        #os.system("iptables -t mangle -I PREROUTING -p udp -d 192.168.0.192 --dport 0:65535 -j TPROXY --on-port 10000 --tproxy-mark 0x01/0x01")
